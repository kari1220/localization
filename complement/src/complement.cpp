...
//pose.pose.position.x~Zがfloat64で格納される pose.pose.orientation.x~wがfloat64で格納される
...
//Localization lcl[N] 格納;
	...
	ros::Subscriber odm_sub = n.subscribe("/tinypower/odom", 100, odom_callback); //knm_tiny_power.cpp
	ros::Subscriber amu_sub = n.subscribe("/AMU_data", 100, amu_callback);
	ros::Subscriber azm_sub = n.subscribe("/perfect_velodyne/filtered_pose", 100, azm_callback);//現在使われてない。
...
//	FILE *fp_1;
	...
	while(ros::ok()){
		if(is_started){
			if(cnt % 50 == 0){
				fprintf(fp_2, "%lf,%lf,%lf,%lf,%lf,%lf\n", lcl[0].x, lcl[0].y, lcl[1].x, lcl[1].y, lcl[2].x, lcl[2].y);//position.csvファイルに書き込み 6つの数字を書き込む
				cnt = 0;
			}
			if(DGauss_flag){//DGaussのcallbackに入ったときtrue,x,y
			...
			if(DGauss_yaw_flag){//DGaussのcallbackに入ったときtrue,z
